...
export interface ConversationRoutingRepository {
   ...
   *   2. pega `pg_advisory_xact_lock(hashtext(tenant_id || ':' || contact_id))` — chave estável de
   *      tenant+contato (não `conversation_id`, que ainda não existe na 1ª mensagem; ver DESVIO 1
   *      em 0016_tenant_router_queue.sql) — serializa a mesma conversa, paraleliza conversas distintas;
